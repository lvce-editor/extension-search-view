...
test('handleMessagePort connects the extension search worker to the renderer process', async () => {
  ...
  await expect(RendererProcess.invoke('Viewlet.queueCommands', 7, [['Viewlet.setValue', 7, 'test']])).resolves.toBe(31)
  expect(queueCommands).toHaveBeenCalledWith(7, [['Viewlet.setValue', 7, 'test']])
