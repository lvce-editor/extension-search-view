...
test('returns extension detail URI for extension ID with special characters', () => {
  const result = getExtensionDetailUri('test@extension#id')
  expect(result).toBe('extension-detail:///test%40extension%23id')
})
...
test('extension detail URI has an empty authority and an encoded path', () => {
  const uri = new URL(getExtensionDetailUri('publisher.name#test'))
  expect(uri.host).toBe('')
  expect(uri.pathname).toBe('/publisher.name%23test')
